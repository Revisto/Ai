...
class Audio():
    ...
    def Music(self):
        ...
        About=['ヾ(⌐■_■)ノ♪ موسیقی','آهنگ شاد ایرانی ~(˘▾˘~) ','یکم شاد بشیم!!!']
        num=(random.randint(1,21))

        Link='static//Media/Music/music_'+str(num)+'.mp3'

        return random.choice(About)+'|||'+Link
        
    def NatureMusic(self):
        
        About=['ヾ(⌐■_■)ノ♪  صدای آرامش بخش طبیعت ヾ(⌐■_■)ノ♪','صدای طبیعت','جند دقیقه آرامش در طبیعت ヾ(⌐■_■)ノ♪ ヾ(⌐■_■)ノ♪','صدای طبیعت برای تمرکز']
        num=(random.randint(1,26))
      
        Link='static//Media/Nature_Sounds/nature_'+str(num)+'.mp3'

        return random.choice(About)+'|||'+Link
      
    def InstrumentalMusic(self):
          
        About=['آهنگ بیکلام','موزیک انگیزشی بی کلام','آهنگ انگیزشی بیکلام']
        num=(random.randint(1,26))
      
        Link='static//Media/Instrumental/instrumental_'+str(num)+'.mp3'
    ...
    def Video(self):
        ...
        vidoal_links=[
        "https://aspb13.cdn.asset.aparat.com/aparat-video/32fa0b35a0daf630bfa0c9a1f8ef2f429583346-720p.mp4",
        "https://aspb11.cdn.asset.aparat.com/aparat-video/bed4aab1db96fd8a9edf91c3f138e9e311960111-720p.mp4",
        "https://aspb13.cdn.asset.aparat.com/aparat-video/1dc2b44c4fc35decaf0dbe5bc1dae3bd7159018-720p.mp4",
        "https://as3.cdn.asset.aparat.com/aparat-video/9675678bc72ebf0dc7afb7dfa2920e676838250-720p.mp4",
        "https://as5.cdn.asset.aparat.com/aparat-video/82009332ee3584b88e77a1ab38fbe2789781375-720p.mp4"
         ]
    
        return random.choice(vidoal_links)
    
        
    def Podcast(self):
        
        About=[
            'ヾ(⌐■_■)ノ♪ پادکست انگیزشی'
            'ヾ(⌐■_■)ノ♪  پادکستی برای افزایش روحیه'
            'ヾ(⌐■_■)ノ♪  پادکست علمی'
        ]
        Podcasts=[
            
        'https://sookhtejet.com/wp-content/uploads/2019/06/Podcast18.mp3',

        'https://sookhtejet.com/wp-content/uploads/2019/06/Podcast16.mp3',

        'https://sookhtejet.com/wp-content/uploads/2019/05/Podcast11.mp3',

        'https://sookhtejet.com/wp-content/uploads/2019/01/8_3SoalMohem.mp3',

        'https://sookhtejet.com/wp-content/uploads/Bonus/Podcast/2_Networking.mp3',
    ...
    def ApiLink(self , Text):
        return "http://api.farsireader.com/ArianaCloudService/ReadTextGET?APIKey=C6QAXK7BNFEI10M&Text="+Text+"&Speaker=Male1&Format=mp3&GainLevel=5&PitchLevel=4&PunctuationLevel=0&SpeechSpeedLevel=5&ToneLevel=0&Quality=low&BeginningSilence=0&EndingSilence=0&Base64Encode=0"
    
    def TTS(self,TEXT,fa="Nothing"):
        TEXT=str(TEXT)
        Url=Audio().ApiLink(TEXT)
        doc = requests.get(Url)
        with open('Audios/Talks/Talk.mp3', 'wb') as f:
            f.write(doc.content)
        Audio().MusicPlay("Talk")
        #with open("TTS.txt","w") as f: 
        #    f.write(str(str(TEXT)+"!@#$"+Url))
        #    f.close()
    ...
    def IDK_Text_Return(self):
        return random.choice(["متاسفانه متوجهِ منظورتون نِمیشم","متاسفانه متوجه نَشدم","شرمنده متوجه منظورتون نِمیشم","شرمنده متوجهِ نشدم","من دارم یاد میگیرم و متوجهِ منظورتون نِمیشم","متاسفانه نفهمیدم"])
...
class Data():
    ...
    def Analyse_Function(self,text,DATA):
        ...
        if DATA["DATA_intents"][BestMatch[1]]["Answers"] != []:
            TheAnswer=random.choice(DATA["DATA_intents"][BestMatch[1]]["Answers"])
            try:
                return TheAnswer+'|||'+DATA["DATA_intents"][BestMatch[1]]["Extra"][0]
            except:
                return TheAnswer
        if DATA["DATA_intents"][BestMatch[1]]["Actions"] != []:
            for Box in DATA["DATA_intents"][BestMatch[1]]["Actions"]:
                #print (Box[0])
                #print (Box[0](*Box[1:len(Box)]))
                TheAnswer=Box[0](*Box[1:len(Box)])
                return TheAnswer
                #return (TheAnswer,Audio().ApiLink(TheAnswer))
        return False
         
    
    def ExtraAi(self,inp,path='Lib/ExtraAi.json'):
    ...
    def Analyse_Closest(self,text,DataBase):
        ...
        if float(BestChoice[0])<=0.65:
            return False
        
        print (BestChoice[2])
        return (BestChoice[2],Audio().ApiLink(BestChoice[2]))
...
class DateAndWeather():
    def Date(self):
        ...
        if str(Date[2])[0]=="0":
            Date[2]=Date[2][1]
        return "روزِ"+str(Date[2])+"اُمِ"+","+"ماهِ"+Months[(int(Date[1])-1)]+","+"سالِ"+str(Date[0])
    ...
    def Time_Now(self):
        ...
        for i in range (len(time)):
            time[i]=int(time[i])
            time[i]=str(time[i])
        return "ساعت"+time[0]+","+"ُّ"+","+time[1]+"دقیقِ"+"ُ"+","+time[2]+"ثانیه"
...
class Others():
    ...
    def Meme(self):
        ...
        Link='static//Media/Images/meme_'+str(num)+'.jpg'
    ...
    def Calculater(self,text):
        ...
        #analizing part
        l_operations=[]
        l_names=[["تقسیم","/"],["ضرب","*"],["جمع","هوالع",'بعلاوه',"علاوه","مجموع","+"],["منفی","منهای","-"]]
        #,["توان"],["رادیکال"]
        l_numbers=[]
        text=text.split()
        for item in text:
        ...
        while "+" in l_full or "-" in l_full:
            ...
            l_full.pop(ind-1)
            l_full.insert(ind-1,sum)
        Text='حاصل عبارت {} است'.format(str(l_full[0]))
        return Text
    ...
    def News(self,Category):
        ...
        Category_New=(list(Category.items()))
        random.shuffle(Category_New)
        for key, url in Category_New:
            allheadlines.extend(get_headlines(url))
            i+=1
            if i==2: break
        Answer=[]
        for hl in range(3) :
            Answer.append(allheadlines[hl])
        return str(("|||".join(map(str, Answer))))
...
class Prices():
    ...
    def RobCoin(self):
        ...
        return ("قیمت ربع سکهْ"+str(text["1/4 Azadi"])+"تومن")
    
    def HalfCoinPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت نیم سکهْ"+str(text["1/2 Azadi"])+"تومن")
    
    def FullCoinPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت سکه بهار آزادیْ"+str(text["1 Old Azadi"])+"تومن")
    
    def DollarPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return (text)
        return ("قیمت دلارْ"+str(text["US Dollar"])+"تومن")
    
    def EuroPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت یُرُ"+str(text["Euro"])+"تومن")
    
    def PondPrice(self): 
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت پوند انگلیسْ"+str(text["British Pound"])+"تومن")
    
    def DerhamPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت دِرهمْ"+str(text["UAE Dirham"])+"تومن")
    
    def LirPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمت لیر ترکیه"+str(text["Turkish Lira"])+"تومن")
    
    def GoldPrice(self):
        text=Prices().GetSomeInfoAboutPrice()
        return ("قیمتِ هر گرم طلای ۱۸ عیارْ "+str(text["Gold 18"])+"تومن")
...
class Corona():
    def Statistics(self,Application):
        ...
        if Application=="Total_Cases":
            return ("تعداد مبتلیان به کرونا"+' '+(str(Total_Cases).replace(",","")))
            
        if Application=="Total_Deaths":
            return ("تعداد مرگ در اثر ابتلا به کرونا"+' '+(str(Total_Deaths).replace(",","")))
            
        if Application=="Total_Recovered":
            return("تعداد بهبودْ یافتگانِ کرونا"+' '+(str(Total_Recovered).replace(",","")))
